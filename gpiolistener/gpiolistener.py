...
class GpioListener(Thread):
    ...
    def __init__(self):
        ...
        self.__states = [
            { "name": "Counting", "value": GpioListener.STATE_COUNTING},
            { "name": "Off", "value": GpioListener.STATE_OFF},
            { "name": "Not working", "value" : GpioListener.STATE_PERMANENT_ON}
        ]
    ...
    def set_relay_alive_time(self, minutes):
        filedata = None
        with open('/home/pi/relaycontroller/config/relayservice.conf', 'r') as file:
            filedata = file.read()

        filedata = filedata.replace('relay_alive_time = %s' % self.relay_alive_time, 'relay_alive_time = %s' % minutes)

        with open('/home/pi/relaycontroller/config/relayservice.conf', 'w') as file:
            file.write(filedata)
    ...
    def run(self):
        try:
            ...
            RelayLogger.get_logger("relayservice", self.logs_dir).info("Started listening on GPIO port %s" %
            ...
            while self.__is_turned_on is True:
                if self.__current_state is GpioListener.STATE_PERMANENT_ON:
                    ...
                    if self.__waiter is None or self.__waiter.IS_RUNNING is False:
            ...
            RelayLogger.get_logger("relayservice", self.logs_dir) \
